...
app.get(`${API_PREFIX}/apfs-organization`, (req, res) => {
    const data = loadData();
    const onlyActive = String(req.query.active ?? '') === '1';

    let rows = data.apfs_organization || [];
    if (onlyActive) rows = rows.filter(r => r.active === 1);

    // stable sort (optional) by full_name then name
...
// GET by id
app.get(`${API_PREFIX}/apfs-organization/:id(\\d+)`, (req, res) => {
    const id = Number(req.params.id);
    const data = loadData();

    const row = (data.apfs_organization || []).find(r => Number(r.id) === id);
    if (!row) return res.status(404).json({ message: 'Org not found' });
...
app.post(`${API_PREFIX}/apfs-organization`, (req, res) => {
    const data = loadData();
    const rows = data.apfs_organization || (data.apfs_organization = []);
    ...
    const row = {
        ...
        acronym: String(payload.acronym || '').trim(),
        full_name: String(payload.full_name || '').trim(),
        active: payload.active === 0 ? 0 : 1,
        parent_id: payload.parent_id === null || payload.parent_id === undefined || payload.parent_id === ''
            ? null
...
// PUT update by id
app.put(`${API_PREFIX}/apfs-organization/:id(\\d+)`, (req, res) => {
    const id = Number(req.params.id);
    const data = loadData();
    const rows = data.apfs_organization || (data.apfs_organization = []);
    ...
    const updated = {
        ...
        acronym: payload.acronym !== undefined ? String(payload.acronym).trim() : current.acronym,
        full_name: payload.full_name !== undefined ? String(payload.full_name).trim() : current.full_name,
        active: payload.active !== undefined ? (payload.active === 0 ? 0 : 1) : current.active,
        parent_id: payload.parent_id !== undefined
            ? (payload.parent_id === null || payload.parent_id === '' ? null : Number(payload.parent_id))
            : current.parent_id,
    };
